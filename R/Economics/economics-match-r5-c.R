...
#  date matcing
#
#  [2017-07-02] - Bulk commodities added
#--------------------------------------------
...
for (i in seq(lookup.rows)) {
    if (lastyear != lookup.datefx[i]$year){
        ...
        ixs_2 <- which((lookup.datefx$year == lastyear)|(lookup.datefx$year == (lastyear-1)))     # Indices of matched items
        # ixs_2 <- which(lookup.datefx$year == lastyear)     # Indices of matched items
        
        tmp <- lookup.dfnew[ixs_2, cn.k.ue]
        xmax.ue <- max(tmp)
        # xmax.ue <- tmp[1]
        xlast.ue <- tmp[length(tmp)]
        
        tmp <- lookup.dfnew[ixs_2, cn.k.aud]
        xmax.aud <- max(tmp)
        # xmax.aud <- tmp[1]
        xlast.aud <- tmp[length(tmp)]
        
        tmp <- lookup.dfnew[ixs_2, cn.k.bulk]
        xmax.bulk <- max(tmp)
        # xmax.bulk <- tmp[1]
        xlast.bulk <- tmp[length(tmp)]
        
        ue.max_24mon <- c(ue.max_24mon, xmax.ue)
        aud.max_24mon <- c(aud.max_24mon, xmax.aud)
        bulk.max_24mon <- c(bulk.max_24mon, xmax.bulk)
        
        ue.last_24mon <- c(ue.last_24mon, xlast.ue)
        aud.last_24mon <- c(aud.last_24mon, xlast.aud)
        bulk.last_24mon <- c(bulk.last_24mon, xlast.bulk)
        ...
        if (lx >= 3) {                                   
            ...
            k1 <- ue.last_24mon[lx-1] - ue.max_24mon[lx-1]
            k2 <- k1 / ue.max_24mon[lx-1]
            k3 <- k2 * (ue.annual[lx-1] - 0.05)
        ...
        if (lx >= 3) {                                   
            ...
            k1 <- aud.last_24mon[lx-1] - aud.max_24mon[lx-1]
            k2 <- k1 / aud.max_24mon[lx-1]
            k3 <- k2 * (aud.annual[lx-1] - 0.5)
        ...
        if (lx >= 3) {                                   
            ...
            k1 <- bulk.last_24mon[lx-1] - bulk.max_24mon[lx-1]
            k2 <- k1 / bulk.max_24mon[lx-1]
            k3 <- k2 * (bulk.annual[lx-1] - 50)
...
write.csv(lookup.dfnew, 'aus-economics-features.csv', row.names=F)
