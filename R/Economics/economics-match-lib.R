# [Readme] ----------------------------
#   <economics-match-lib.r>
#   - Based on <unemploy-fx-match-lib.r>
...
# Global varibles:
lookup.Economics <- NULL     

# Function <matchUempFx()>
matchEconomics <- function(data = NULL, 
                           datename = "Date",
                           dateformat = "%Y-%m-%d") {
    
    lookup.csv <- "aus-economics.csv"  # file name of source data of lookup table
    # lookup.dateformat <- "%b-%y"      # date format string for lookup, like'May-16'
    ...
    lookup.dfnew <- data.frame(Date = lookup.df[,"Date"],                            # Date (mm-yy)
                               ...
                               Date.Bulk = lookup.df[,"Date.Bulk"],                      # Date.FX (dd-mm-yy)
                               Bulk.USD = lookup.df[,"Bulk.USD"],                        # AUDUSD
                               Bulk.Annual = rep(NA, lookup.rows),                 # AUDUSD.Annual
                               Bulk.K1 = rep(NA, lookup.rows),                     # AUDUSD.K1
                               Bulk.K2 = rep(NA, lookup.rows),                     # AUDUSD.K2
                               Bulk.K3 = rep(NA, lookup.rows),                     # AUDUSD.K3
    ...
    lookup.date <- strptime(lookup.df[,"Date.FX"], lookup.dateformat)
    ue.annual <- NULL                                       # save annual value of unemployment
    aud.annual <- NULL                                      # save annual value of AUDUSD
    bulk.annual <- NULL                                     # save annual value of Bulk.USD
    lastyear <- 0
    for (i in seq(lookup.rows)) {
        if (lastyear != lookup.date[i]$year){
            lastyear <- lookup.date[i]$year
            
            ixs <- which(lookup.date$year == lastyear)       # Indices of matched items
    ...
    for (i in seq(lookup.rows)) {
        ...
        if (length(ixs) > 0){                                    # index set available?
            ...
            target.dfnew[ixs, cn.t.bulkk2] <- k.row[, cn.k.bulkk2] # copy "Bulk.K1"
            target.dfnew[ixs, cn.t.bulkk3] <- k.row[, cn.k.bulkk3] # copy "Bulk.K1"
        }
    }
