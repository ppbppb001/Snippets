#-------------------------------------------
# <economics-match-r6.r>
#  Improve the target table with 
#  unemployment and FX data colmum
#  date matcing
#
#  [2016-01-02] - r5: Bulk commodities added
#  [2016-01-12] - r6: slide window by month
#--------------------------------------------
...
cn.k.uek4 <- "Unemployment.K4"        # 'Unemployment.K4' = ue[i-1] - max(ue[i-1]..ue[i-12])
cn.k.datefx <- "Date.FX"
cn.k.aud <- "AUDUSD"
...
cn.k.audk4 <- "AUDUSD.K4"             # 'AUDUSD.K4' = aud[i-1] - max(aud[i-1]..aud[i-12])
cn.k.datebulk <- "Date.Bulk"
cn.k.bulk <- "Bulk.USD"
cn.k.bulka <- "Bulk.Annual"
cn.k.bulkk1 <- "Bulk.K1"              # 'Bulk.K1' = Value.Year(i-1) - Value.Year(i-2)
cn.k.bulkk2 <- "Bulk.K2"              # 'Bulk.K2' = K1/Value.Year(i-2)
cn.k.bulkk3 <- "Bulk.K3"              # 'Bulk.K3' = K2*(Value.Year(i-1) - 0.5)
cn.k.bulkk4 <- "Bulk.K4"              # 'Bulk.K4' = bulk[i-1] - max(bulk[i-1]..bulk[i-12])
...
# *******************************************************
#
#   Make economics lookup table
#
# *******************************************************

# Load unemployment and Fx historical data: ..................
lookup.df <- read.csv("aus-economics.csv", stringsAsFactors = FALSE) 
lookup.rows <- dim(lookup.df)[1]


# Make a new lookup dataframe:
lookup.dfnew <- data.frame(Date = lookup.df[,"Date"],                            # Date (mm-yy)
                           ...
                           Unemployment.K4 = rep(NA, lookup.rows),               # Unemployment.K4
                           ...
                           AUDUSD.K4 = rep(NA, lookup.rows),                     # AUDUSD.K4
                           Date.Bulk = lookup.df[,"Date.Bulk"],                  # Date.Bulk (mm-yy)
                           Bulk.USD = lookup.df[,"Bulk.USD"],                    # Bulk.USD
                           Bulk.Annual = rep(NA, lookup.rows),                   # Bulk.Annual
                           Bulk.K1 = rep(NA, lookup.rows),                       # Bulk.K1
                           Bulk.K2 = rep(NA, lookup.rows),                       # Bulk.K2
                           Bulk.K3 = rep(NA, lookup.rows),                       # Bulk.K3
                           Bulk.K4 = rep(NA, lookup.rows),                       # Bulk.K4
...
bulk.annual <- NULL                                     # save annual value of Bulk.USD
lastyear <- 0
for (i in seq(lookup.rows)) {
    ...
    if (lastyear != lookup.datefx[i]$year){
        ...
        xmean.bulk <- mean(lookup.dfnew[ixs, cn.k.bulk])
        lookup.dfnew[ixs, cn.k.bulka] <- xmean.bulk      # Annual mean of Bulk.USD
        
        ue.annual <- c(ue.annual, xmean.ue)              # Append annual value
        aud.annual <- c(aud.annual, xmean.aud)           # Append annual value
        bulk.annual <- c(bulk.annual, xmean.bulk)        # Append annual value
        ...
        # Bulk.K1/K2/K3:
        lx <- length(bulk.annual)
        if (lx >= 3) {                                   
            # Calculate Bulk.K1/K2/K3
            k1 <- bulk.annual[lx-1] - bulk.annual[lx-2]
            k2 <- k1 / bulk.annual[lx-2]
            k3 <- k2 * (bulk.annual[lx-1] - 50)
            # Assign Bulk.K1/K2/K3
            lookup.dfnew[ixs, cn.k.bulkk1] <- k1
            lookup.dfnew[ixs, cn.k.bulkk2] <- k2
            lookup.dfnew[ixs, cn.k.bulkk3] <- k3
        }
    ...
    # Monthly .....................................
    if (i >= 13) {
        ixs <- (i-12):(i-1)  # index/month[i-1] .. [i-12]
        
        # Unemployment.K4
        uek4 <- lookup.dfnew[(i-1), cn.k.ue] - max(lookup.dfnew[ixs, cn.k.ue])
        lookup.dfnew[i, cn.k.uek4] <- uek4
        
        # AUDUSD.K4
        audk4 <- lookup.dfnew[(i-1), cn.k.aud] - max(lookup.dfnew[ixs, cn.k.aud])
        lookup.dfnew[i, cn.k.audk4] <- audk4
        
        # Bulk.K4
        bulkk4 <- lookup.dfnew[(i-1), cn.k.bulk] - max(lookup.dfnew[ixs, cn.k.bulk])
        lookup.dfnew[i, cn.k.bulkk4] <- bulkk4
    }
...
# *******************************************************
#
#   Matching by checking lookup table
#
# *******************************************************
...
target.dfnew <- data.frame(target.df,                                       # Original target
                           ...
                           Unemployment.K4 = rep(NA, target.rows),          # Unemployment.K4
                           ...
                           AUDUSD.K4 = rep(NA, target.rows),                # AUDUSD.K4
                           Bulk.USD = rep(NA, target.rows),                 # Bulk.USD
                           Bulk.Annual = rep(NA, target.rows),              # Bulk.Annual
                           Bulk.K1 = rep(NA, target.rows),                  # Bulk.K1
                           Bulk.K2 = rep(NA, target.rows),                  # Bulk.K2
                           Bulk.K3 = rep(NA, target.rows),                  # Bulk.K3
                           Bulk.K4 = rep(NA, target.rows),                  # Bulk.K4
...
for (i in seq(lookup.rows)) {
    ...
    if (length(ixs) > 0){                                    # index set available?
        ...
        target.dfnew[ixs, cn.t.uek4] <- k.row[, cn.k.uek4]   # copy "Unemployment.K4"
        ...
        target.dfnew[ixs, cn.t.audk2] <- k.row[, cn.k.audk2] # copy "AUDUSD.K2"
        target.dfnew[ixs, cn.t.audk3] <- k.row[, cn.k.audk3] # copy "AUDUSD.K3"
        target.dfnew[ixs, cn.t.audk4] <- k.row[, cn.k.audk4] # copy "AUDUSD.K4"
        
        target.dfnew[ixs, cn.t.bulk] <- k.row[, cn.k.bulk]     # copy "Bulk.USD"
        target.dfnew[ixs, cn.t.bulka] <- k.row[, cn.k.bulka]   # copy "Bulk.Annual"
        target.dfnew[ixs, cn.t.bulkk1] <- k.row[, cn.k.bulkk1] # copy "Bulk.K1"
        target.dfnew[ixs, cn.t.bulkk2] <- k.row[, cn.k.bulkk2] # copy "Bulk.K2"
        target.dfnew[ixs, cn.t.bulkk3] <- k.row[, cn.k.bulkk3] # copy "Bulk.K3"
        target.dfnew[ixs, cn.t.bulkk4] <- k.row[, cn.k.bulkk4] # copy "Bulk.K4"
    }
}
