# <glm-comparison-lib.r> -------------
#
#  functions for glm-comparison
#  
# [2017-07-09]:
#
#------------------------------------------
...
glmPredAndEval <- function(data.train = NULL,       # data frame as training set
    ...
    ref <- data.test[, name.ref]            # Set reference data
    
    # [Balance train data] --------------------------
    if (!is.null(balance)) {
        data.train <- binaryBalance(data = data.train,
                                    name.class = name.ref,
                                    balance = balance)
    }
    ...
    output$table <- list(table)            # push to output list
    ...
    rocr.perf.tprfpr <- performance(rocr.pred, 
                                    measure = "tpr", 
                                    x.measure = "fpr")
    output$perf.tprfpr <- rocr.perf.tprfpr     # push to output list
    
    # recall:
    rocr.perf.recall <- performance(rocr.pred, 
                                    measure = "prec", 
                                    x.measure = "rec")
    output$perf.recall <- rocr.perf.recall     # push to output list
    
    # AUC:
    rocr.perf.auc <- performance(rocr.pred, 
                                 measure = "auc")
    output$perf.auc <- rocr.perf.auc           # push to output list
    
    # acc/err:
    rocr.perf.accerr <- performance(rocr.pred, 
                                    measure = "acc", 
                                    x.measure = "err")
    output$perf.accerr <- rocr.perf.accerr     # push to output list
